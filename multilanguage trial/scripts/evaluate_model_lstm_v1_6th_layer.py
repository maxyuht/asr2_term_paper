...
# ✅ 加载测试数据
test_set = FeatureEmotionDataset("./splits_lstm_v1_6th_layer/test")
test_loader = DataLoader(test_set, batch_size=16, shuffle=False, collate_fn=collate_fn_lstm)

# ✅ 加载模型
model = EmotionLSTMClassifier()
model.load_state_dict(torch.load("./models_lstm_v1_6th_layer/best_model_lstm_v1_6th_layer.pt", map_location=device))
model.to(device)
model.eval()
...
# ✅ 保存报告
with open("test_classification_report_lstm_v1_6th_layer.txt", "w") as f:
...
plt.grid(False)
plt.savefig("test_confusion_matrix_lstm_v1_6th_layer.png")
plt.show()
